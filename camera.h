...
/* In path tracing, the "camera" or "image plane" is a virtual rectangle upon which the 3D scene
is projected. The class `Camera` encapsulates the notion of a camera along with a designated
camera/eye point. */
class Camera {

    Camera(double w, double h, size_t img_w, size_t img_h, const Point3D &camera_center_,
             double focal_length_)
      : width{w}, height{h},
        /* Right-handed coordinates: The y-axis goes up, the x-axis goes right, the NEGATIVE z-axis
        points from the camera toward the image. Thus, the vector going down the camera (`y_vec`)
...
public:
    ...
    /* Width and height of the camera. Note that they are real-valued. */
    double width, height;
    /* Vectors right and down across the camera horizontal and vertical edges. */
    Vec3D x_vec, y_vec;
    /* The horizontal and vertical delta vectors from pixel to pixel. */
    Point3D pixel_delta_x, pixel_delta_y;
    /* Coordinates of the camera/eye point and its distance away from the camera */
    Point3D camera_center;
    double focal_length;
    /* Upper left coordinate of the camera, and the coordinates of the top-left image pixel */
    ...
    static auto from_width_and_image(double w, const Image &img, const Point3D &camera_center = {},
        ...
        /* Camera heights less than 1 are ok because they are real-valued, unlike Image heights.
        ...
        return Camera(w, w / img.aspect_ratio(), img.width(), img.height(), camera_center,
                        focal_length);
    }

    static auto from_height_and_image(double h, const Image &img, const Point3D &camera_center = {},
                                      double focal_length = 1) {
        /* Camera widths less than 1 are ok because they are real-valued, unlike Image widths
        Also note that we must use `img.aspect_ratio()` and not the theoretical desired ratio, because
        the ideal ratio may not be the actual aspect ratio of `img`, because `img.width()` and
        `img.height()` both must be integers. */
        return Camera(h * img.aspect_ratio(), h, img.width(), img.height(), camera_center,
                        focal_length);
    }
